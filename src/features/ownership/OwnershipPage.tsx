...
import { Target, PieChart, Coins, Info, Crown } from 'lucide-react';
...
function OwnershipBar({ frac, minLog, maxLog, color }: { frac: number; minLog: number; maxLog: number; color: string }) {
  const logF = Math.log10(frac);
  const span = maxLog - minLog;
  const fill = span === 0 ? 1 : Math.max(0.02, (logF - minLog) / span);
...
export default function OwnershipPage() {
  ...
  const logs = rows.map((r) => Math.log10(r.frac));
  const minLog = Math.min(...logs);
  const maxLog = Math.max(...logs);
  const totalShares = rows.reduce((s, r) => s + r.yours, 0);
  const largest = rows[0];               // max frac -> min N
  const smallest = rows[rows.length - 1]; // min frac -> max N
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <StatCard icon={Target} label="Largest stake" value={`1 in ${fmtN(largest.N)}`}
          sub={`${largest.ticker} · ${largest.name ?? ''}`} accent={TIER_COLOR.Meaningful} />
      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500">
                ...
                <th className="text-right px-3.5 py-2.5 font-bold">Your shares</th>
              ...
              {rows.map((r, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-3.5 py-3 text-right text-[12.5px] font-medium text-slate-900 dark:text-white tabular-nums">{r.yours}</td>
                    ...
                    <td className="px-5 py-3 text-right whitespace-nowrap">
                      <div className="inline-flex items-baseline gap-1 tabular-nums">
                        ...
                        <span className="text-[16px] font-bold text-slate-900 dark:text-white tracking-tight">{fmtN(r.N)}</span>
                      </div>
                    </td>
                  </tr>
                );
        ...
        <div className="flex flex-wrap gap-3.5 items-center px-5 py-2.5 border-t border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/40 text-[11px] text-slate-500 dark:text-slate-400">
          ...
          <span className="ml-auto tabular-nums">Outstanding shares as of last provider update</span>
        </div>
      </div>
    </div>
  );
